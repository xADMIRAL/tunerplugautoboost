...
| `als_iac_steps` (воздух РХХ, стартовое) | 90 | 100 | 140 |
...
* **Do not let RPM fall below** — обороты, ниже которых антилаг не даёт мотору упасть на
  закрытом газе. Ручка — воздух через РХХ (`als_iac_steps` для шагового клапана, `als_iac_duty`
  для ШИМ): обороты просели ниже — воздуха больше; держатся сильно выше — чуть меньше (меньше
  продувки и тепла). В ECU при этом пишется `als_minrpm` = удержание − 700 (отсечка антилага
  заведомо ниже удерживаемых оборотов).
